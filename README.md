...
DJI Osmo Pocket/Action cameras automatically generate low resolution versions of the original videos captured. These videos are saved by the camera as .LRF files (Low Resolution File). They are intended to be used for viewing playback footage on the camera or DJI Mimo app.

This script takes the low resolution versions generated by the DJI camera and uses them in your Shotcut project as proxy clips. This saves you the processing time and resources required to generate the proxy clips with Shotcut!
...
* Add your high resolution DJI videos to the project
![Add Playlist](./screenshots/add_playlist.png)
* Run the [ShotcutLoadGoProProxies.py](./ShotcutLoadDJIProxies.py) Python script, specifying the `--shotcut-path` parameter as the path to your Shotcut project directory (which includes the `.mlt` project file)
```
./ShotcutLoadDJIProxies.py --shotcut-path <SHOTCUT_PROJECT_DIRECTORY>
INFO:Retrieving original video files from Shotcut project...
INFO:Searching for low resolution versions of video files (.LRF)...
...
* Observe that `(PROXY)` was added to the names of clips for which a low resolution DJI version was found.
